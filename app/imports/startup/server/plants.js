...
const plantSeeds = [
  {
    ...
    /* addedBy: Meteor.users.findOne({ username: Meteor.settings.defaultAccount.profile.name })._id, */
    addedBy: Meteor.settings.defaultAccount.profile.name,
  },
];
...
if (Plants.find().count() === 0) {
  _.each(plantSeeds, function seedPlants(plant) {
    console.log ('The owner is ' + Meteor.settings.defaultAccount.profile.name /* Meteor.users.findOne({ username: Meteor.settings.defaultAccount.username })._id*/);
    Plants.insert(plant);
  });
}
